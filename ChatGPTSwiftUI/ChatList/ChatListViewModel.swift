...
class ChatListViewModel : ObservableObject {
    ...
    func fetchData (user:String?) {
        //self.chats = [AppChat(id: "1", topic: "Topic", model: .gpt3_5_turbo, lastMessageSent: Date(), owner: "123"),
        //AppChat(id: "2", topic: "Topic some", model: .gpt4, lastMessageSent: Date(), owner: "123")]
        //self.loadingState = .resultFound
        if loadingState == .none{
            loadingState = .loading
            db.collection("chats").whereField("owner",isEqualTo: user ?? "" ).addSnapshotListener{
